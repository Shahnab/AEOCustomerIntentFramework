...
const GlassCard: React.FC<GlassCardProps> = ({ children, className = "", title, action, noPadding = false }) => {
  return (
    <div className={`flex flex-col rounded-lg border border-white/20 bg-surface shadow-sm overflow-hidden ${className}`}>
        {(title || action) && (
            <div className="flex items-center justify-between px-5 py-3 border-b border-white/10 min-h-[50px] shrink-0 bg-surface z-10">
                {title && (
        ...
        <div className={`relative flex-1 min-h-0 flex flex-col ${noPadding ? '' : 'p-5'}`}>
            {children}
        </div>
    </div>
  );
